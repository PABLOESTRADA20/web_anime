...
export default function Admin() {
  ...
  async function handleApprove(id) {
    try { await updateStatus(id, 'approved'); toast.success('Aprobado') }
    catch (err) { toast.error(err.message) }
  }

  async function handleReject(id) {
    try { await updateStatus(id, 'rejected'); toast.success('Rechazado') }
    catch (err) { toast.error(err.message) }
  }

  async function handleDelete(id) {
    if (!confirm('¿Eliminar este enlace permanentemente?')) return
    try { await removeEpisode(id); toast.success('Eliminado') }
    catch (err) { toast.error(err.message) }
  }
  ...
  if (adminLoading) return <div className="max-w-4xl mx-auto"><div className="h-8 w-48 bg-surface rounded-lg animate-pulse mb-6" /><div className="space-y-3">{Array.from({ length: 5 }).map((_, i) => <div key={i} className="h-20 bg-surface rounded-xl animate-pulse" />)}</div></div>
    ...
    return (
      <>
        ...
        <div className="max-w-xl mx-auto py-12 text-center">
          <GradientHeading variant="pink" size="lg" className="mb-4">Acceso restringido</GradientHeading>
          <p className="text-text-secondary mb-6">No tienes permisos de administrador.</p>
          <button onClick={handleBootstrap} className="px-6 py-3 rounded-xl bg-primary text-white text-sm hover:bg-primary-hover transition-colors">
    ...
    <>
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <GradientHeading variant="pink" size="lg">Administración</GradientHeading>
          <button onClick={refetch} className="px-4 py-2 rounded-lg text-xs bg-surface text-text-secondary hover:text-text-primary border border-white/10 hover:border-primary/30 transition-all">
            Recargar
          </button>
        </div>
        ...
        <div className="flex gap-1 mb-6 overflow-x-auto pb-1">
          {STATUS_TABS.map(t => (
            <button key={t.key} onClick={() => setTab(t.key)}
              ...
              {t.label} <span className={tab === t.key ? 'text-white/70' : t.color}>({episodes.filter(e => e.status === t.key).length})</span>
            </button>
          ))}
        </div>

        {modLoading ? (
          <div className="space-y-3">{Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-24 bg-surface rounded-xl animate-pulse" />)}</div>
        ) : filtered.length === 0 ? (
          <div className="text-center py-16 text-text-secondary">No hay episodios {tab === 'pending' ? 'pendientes' : tab === 'approved' ? 'aprobados' : 'rechazados'}.</div>
        ) : (
                ...
                <div className="flex items-start gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      <Link to={`/watch?anilistId=${ep.anilist_id}&ep=${ep.episode_number}`} className="text-sm font-medium hover:text-primary transition-colors truncate">
                        {ep.title || `Anime #${ep.anilist_id}`}
                      </Link>
                      <span className="text-[10px] text-text-secondary shrink-0">Ep. {ep.episode_number}</span>
                      <span className={`text-[10px] px-1.5 py-0.5 rounded ${getProviderColor(ep.provider_name)}`}>{getProviderLabel(ep.provider_name)}</span>
                      <span className="text-[10px] px-1.5 py-0.5 rounded bg-surface text-text-secondary border border-white/10">{getLanguageLabel(ep.language)}</span>
                    </div>
                    <a href={ep.url} target="_blank" rel="noopener noreferrer" className="text-[11px] text-text-secondary/60 hover:text-neon-cyan truncate block max-w-md">
                      {ep.url}
                    </a>
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    {ep.status !== 'approved' && (
                      <button onClick={() => handleApprove(ep.id)}
                    ...
                    {ep.status !== 'rejected' && (
                      <button onClick={() => handleReject(ep.id)}
