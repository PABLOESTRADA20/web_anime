...
export default function CollectionDetail() {
  ...
  async function handleRemove(itemId) {
    try { await removeItem(itemId); toast.success('Eliminado de la colección') }
    catch (err) { toast.error(err.message) }
  }

  if (collLoading || itemsLoading) return <div className="max-w-4xl mx-auto"><div className="h-8 w-48 bg-surface rounded-lg animate-pulse mb-6" /><div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3">{Array.from({ length: 10 }).map((_, i) => <div key={i} className="aspect-[3/4] bg-surface rounded-xl animate-pulse" />)}</div></div>
    ...
    <>
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex items-start justify-between mb-6">
          <div>
            <Link to="/collections" className="text-xs text-text-secondary hover:text-primary transition-colors mb-1 inline-block">← Mis colecciones</Link>
            ...
            <div className="flex gap-3 mt-2 text-xs text-text-secondary">
              ...
              {collection.is_public ? <span className="text-green-400/60">Pública</span> : <span className="text-yellow-400/60">Privada</span>}
            </div>
          </div>
        </div>
        ...
        {items.length === 0 ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
            {items.map(item => {
              ...
              return (
                <div key={item.id} className="group relative rounded-xl overflow-hidden bg-surface hover:bg-surface-hover transition-colors">
                  <Link to={link}>
