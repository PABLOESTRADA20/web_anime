...
import { useCommunityEpisodes, useSubmitEpisode, useEpisodeVote, getProviderLabel, getProviderColor, getLanguageLabel } from '../hooks/useCommunityEpisodes'
...
export default function CommunityEpisodes({ anilistId, episodeNumber, title, onSelectUrl }) {
  ...
  async function handleSubmit(e) {
    e.preventDefault()
    setFormError('')
    if (!form.url) { setFormError('La URL es obligatoria'); return }
    if (!form.url.startsWith('http://') && !form.url.startsWith('https://')) { setFormError('URL inválida'); return }
    try {
      await submit({
          ...
          {links.map((link) => (
            ...
            >
              <div className="flex flex-col items-center gap-0.5 min-w-[32px]">
                ...
                <span className={`text-xs font-mono font-bold ${link.votes > 0 ? 'text-green-400' : link.votes < 0 ? 'text-red-400' : 'text-text-secondary'}`}>
                  {link.votes}
                </span>
                <button
                  onClick={() => handleVote(link.id, -1)}
              ...
              <div className="flex-1 min-w-0">
                ...
                {link.title && (
                  <p className="text-xs text-text-secondary truncate mt-1">{link.title}</p>
                )}
              </div>
              <button
                onClick={() => {
