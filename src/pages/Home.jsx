  ...
  return (
    <div className="flex items-center justify-between mb-5">
      <div className="flex items-center gap-3">
        ...
        <GradientHeading variant="pink" size="sm">{title}</GradientHeading>
      </div>
      <Link to={link} className="text-xs text-neon-cyan hover:text-primary transition-colors font-medium">{linkText}</Link>
    </div>
  )
}
  ...
  return (
    ...
    >
        ...
        <section className="mb-12 space-y-8">
          {recentHistory.length > 0 && (
            <FadeIn>
              <div>
                <SectionHeader
                  icon={<svg className="w-4 h-4 text-neon-cyan" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>}
                  ...
                  {mangaHistory.slice(0, 8).map((item) => (
                    <div key={item.id} className="flex items-center gap-4 p-3 rounded-xl bg-surface hover:bg-surface-hover transition-all duration-300 group border border-transparent hover:border-primary/20 card-hover">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <Link to={`/manga/${item.anilist_id}/read?chapterId=${item.chapter_id}&chapter=${item.chapter_number}&title=${encodeURIComponent(item.title || '')}&image=${encodeURIComponent(item.image || '')}`}
                          className="inline-flex items-center gap-1 mt-1 text-xs text-text-secondary hover:text-neon-cyan transition-colors"
                        >
      ...
      <section className="mb-12">
        <FadeIn>
          <SectionHeader
            icon={<svg className="w-4 h-4 text-neon-cyan" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>}
            title="Más populares"
            link="/search"
            linkText="Ver más"
          />
      ...
      <section className="mb-12">
        <FadeIn>
          <SectionHeader
            icon={<svg className="w-4 h-4 text-accent" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>}
      ...
      <section className="mb-12">
        <FadeIn>
          <SectionHeader
            icon={<svg className="w-4 h-4 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" /></svg>}
                ...
                <FadeIn key={`${ch.mangaId}-${ch.chapterNumber}`}>
                  ...
                  >
                    <div className="aspect-[3/4] overflow-hidden">
                      {ch.coverUrl ? (
                        <img src={ch.coverUrl} alt={ch.mangaTitle} loading="lazy"
                          className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110"
                        />
                      ) : (
                        <div className="w-full h-full bg-surface-hover flex items-center justify-center text-text-secondary text-xs">Sin imagen</div>
                      )}
                    </div>
                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent pointer-events-none" />
                    {ch.format && (
                      <span className="absolute top-2 left-2 bg-black/60 backdrop-blur-sm text-primary text-[10px] font-mono font-semibold px-2 py-1 rounded-lg border border-primary/20 tracking-wider uppercase">{ch.format}</span>
                    )}
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-3">
                      <h3 className="text-sm font-heading font-semibold text-white line-clamp-2 leading-tight drop-shadow-lg">{ch.mangaTitle}</h3>
                    </div>
