...
import {
  useCommunityNovelChapters,
  useSubmitNovelChapter,
  useNovelChapterVote,
  getNovelProviderLabel,
  getNovelProviderColor,
} from '../hooks/useCommunityNovels'
import { useToast } from './Toast'
import EmptyState from './EmptyState'

export default function CommunityNovelChapters({ novelSlug, chapterNumber, title }) {
  const { user } = useAuth()
  const toast = useToast()
  const { links, loading, refetch } = useCommunityNovelChapters(novelSlug, chapterNumber)
  const { submit, submitting } = useSubmitNovelChapter()
  const { vote, voting } = useNovelChapterVote()
          ...
          <motion.form
            ...
            <div>
              ...
              <input
                ...
                placeholder="https://novelbin.net/..."
            ...
            <div
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${getNovelProviderColor(link.provider_name)}`}>
                    {getNovelProviderLabel(link.provider_name)}
                  </span>
                </div>
