...
async function handleGet(url: URL, _user: string | null) {
  ...
  const data = await restGet('community_manga_chapters', params)
...
async function handlePost(body: Record<string, unknown>, user: string | null) {
  ...
  if (action === 'submit') {
    ...
    const { anilist_id, chapter_number, url, provider_name, title } = body as unknown as MangaChapterLink
    if (!anilist_id || !chapter_number || !url || !provider_name) {
      return new Response(JSON.stringify({ error: 'Missing required fields: anilist_id, chapter_number, url, provider_name' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }

    const data = await restPost('community_manga_chapters', {
      anilist_id,
      chapter_number,
      title: title || '',
      url,
  ...
  if (action === 'vote') {
    ...
    if (existing?.length > 0) {
      if (existing[0].vote === vote) {
        await fetch(`${SUPABASE_URL}/rest/v1/community_manga_chapter_votes?id=eq.${existing[0].id}`, {
          method: 'DELETE',
          headers: {
            Authorization: `Bearer ${SUPABASE_SERVICE_ROLE_KEY}`,
            apikey: SUPABASE_SERVICE_ROLE_KEY,
      ...
      await restPost('community_manga_chapter_votes', {
        user_id: user,
    ...
    const votesRes = await fetch(`${SUPABASE_URL}/rest/v1/community_manga_chapter_votes?chapter_id=eq.${chapterId}&select=vote`, {
      headers: {
        Authorization: `Bearer ${SUPABASE_SERVICE_ROLE_KEY}`,
        apikey: SUPABASE_SERVICE_ROLE_KEY,
      },
    ...
    await fetch(`${SUPABASE_URL}/rest/v1/community_manga_chapters?id=eq.${chapterId}`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${SUPABASE_SERVICE_ROLE_KEY}`,
