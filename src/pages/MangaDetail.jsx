      ...
      <div className="flex flex-col sm:flex-row gap-6 mb-10">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap gap-2 mt-3">
            ...
            {manga.format && (
              <span className="text-xs bg-accent/20 text-accent px-3 py-1 rounded-full">{manga.format}</span>
            )}
            {manga.chapters && (
            ...
            {manga.status && (
              <span className="text-xs bg-surface px-3 py-1 rounded-full text-text-secondary">
                {manga.status === 'RELEASING' ? 'Publicándose' : manga.status === 'FINISHED' ? 'Finalizado' : manga.status === 'NOT_YET_RELEASED' ? 'Próximamente' : manga.status}
              </span>
            )}
          </div>

          {manga.synonyms?.length > 0 && (
            <p className="text-xs text-text-secondary mt-3">Sinónimos: {manga.synonyms.join(', ')}</p>
          )}

          {manga.tags?.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mt-3">
              {manga.tags.slice(0, 8).map((t) => (
                <span key={t.name} className="text-[10px] bg-surface px-2 py-0.5 rounded text-text-secondary">{t.name}</span>
              ))}
            </div>
          )}
          ...
          {manga.staff?.edges?.length > 0 && (
            <p className="text-sm text-text-secondary mt-3">
              Staff: {manga.staff.edges.slice(0, 3).map((s) => s.node.name.full).join(', ')}
            </p>
          )}

          <div className="flex gap-3 mt-6">
            <Link
              to={readChapter ? `/manga/${id}/read?chapterId=${readChapter.chapterId}&chapter=${readChapter.chapterNumber}&title=${encodeURIComponent(title)}&image=${encodeURIComponent(image || '')}` : '#'}
              ...
              onClick={(e) => { if (!readChapter) e.preventDefault() }}
            >
        ...
        <section className="mb-10">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
            {manga.relations.edges.slice(0, 6).map((edge, i) => {
              ...
              return (
                <Link key={rel.id} to={`/manga/${rel.id}`} className="group relative rounded-2xl overflow-hidden bg-surface hover:bg-surface-hover transition-all">
                  <div className="aspect-[3/4] overflow-hidden">
                    <img src={rel.coverImage?.large} alt="" className="w-full h-full object-cover" />
                  </div>
                  <div className="p-2">
        ...
        <section className="mb-10">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
            {manga.recommendations.nodes.slice(0, 6).map((rec, i) => {
              ...
              return (
                <Link key={media.id} to={`/manga/${media.id}`} className="group relative rounded-2xl overflow-hidden bg-surface hover:bg-surface-hover transition-all">
                  <div className="aspect-[3/4] overflow-hidden">
                    <img src={media.coverImage?.large} alt="" className="w-full h-full object-cover" />
                  </div>
                  <div className="p-2">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {ch.title && (
                        <p className="text-xs text-text-secondary truncate">{ch.title}</p>
                      )}
                    </div>
                    {read && (
                      <span className="text-[10px] text-text-secondary shrink-0">Leído</span>
                    )}
                  </Link>
                )
              })}
