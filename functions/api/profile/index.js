...
export async function onRequest(context) {
  ...
  if (method === 'PUT') {
    ...
    const { display_name, bio, avatar_url, website, xp, achievement_id } = body
    ...
    if (xp !== undefined && xp !== null) {
      ...
      await execute(
        env.DB,
        "UPDATE user_profiles SET xp = ?, level = ?, xp_updated_at = datetime('now') WHERE id = ?",
    ...
    await execute(
      env.DB,
      "INSERT INTO user_profiles (id, display_name, bio, avatar_url, website) VALUES (?, ?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET display_name = COALESCE(?, display_name), bio = COALESCE(?, bio), avatar_url = COALESCE(?, avatar_url), website = COALESCE(?, website), updated_at = datetime('now')",
      [user.id, display_name || '', bio || '', avatar_url || '', website || '', display_name || null, bio || null, avatar_url || null, website || null],
    )
