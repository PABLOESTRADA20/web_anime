...
export async function onRequest(context) {
  ...
  if (method === 'GET') {
    const items = await query(
      env.DB,
      'SELECT * FROM anime_favorites WHERE user_id = ? ORDER BY created_at DESC',
      [user.id],
    )
  ...
  if (method === 'POST') {
    ...
    await execute(
      env.DB,
      'INSERT OR IGNORE INTO anime_favorites (user_id, anilist_id, title, image) VALUES (?, ?, ?, ?)',
  ...
  if (method === 'DELETE') {
    ...
    await execute(
      env.DB,
      'DELETE FROM anime_favorites WHERE user_id = ? AND anilist_id = ?',
      [user.id, anilist_id],
    )
    return new Response(null, { status: 204, headers: corsHeaders() })
  }
