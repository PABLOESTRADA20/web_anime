...
export function useNovelRatings() {
  ...
  const fetchRating = useCallback(async (slug) => {
    ...
    if (!token) return
    try {
      const res = await fetch(`/api/novel/ratings?novel_slug=${encodeURIComponent(slug)}`, {
  ...
  async function setRating(slug, rating) {
    ...
    if (existing === rating) {
      ...
      const res = await fetch('/api/novel/ratings', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ novel_slug: slug, rating }),
      })
      if (res.ok) {
        const { data } = await res.json()
        if (data?.rating) setRatings((prev) => ({ ...prev, [slug]: data.rating }))
