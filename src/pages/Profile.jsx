...
export default function Profile() {
  ...
  ], [watching.length, planToWatch.length, completed.length, reading.length, planToRead.length, completedManga.length, watchlist.length, favorites.length, history.length, mangaHistory.length])
      ...
      {section === 'reading' && (
        <section>
          {reading.length === 0 ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              {reading.map((item) => (
                <Link key={item.id} to={`/manga/${item.anilist_id}`} className="group rounded-xl overflow-hidden bg-surface hover:bg-surface-hover transition-colors">
                  <div className="aspect-[3/4] overflow-hidden"><img src={item.image} alt={item.title} className="w-full h-full object-cover" loading="lazy" /></div>
                  <p className="text-xs p-2 line-clamp-2">{item.title}</p>
                </Link>
              ))}
            </div>
