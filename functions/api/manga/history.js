...
export async function onRequest(context) {
  ...
  if (method === 'GET') {
    const items = await query(
      env.DB,
      'SELECT * FROM manga_history WHERE user_id = ? ORDER BY updated_at DESC LIMIT 50',
      [user.id],
    )
  ...
  if (method === 'POST') {
    ...
    const existing = await queryOne(
      env.DB,
      'SELECT id FROM manga_history WHERE user_id = ? AND anilist_id = ? AND chapter_number = ?',
      [user.id, anilist_id, chapter_number],
    )

    if (existing) {
      await execute(
        env.DB,
        'UPDATE manga_history SET updated_at = datetime(\'now\'), page = ? WHERE id = ?',
        [page ?? 1, existing.id],
      )
    } else {
      await execute(
