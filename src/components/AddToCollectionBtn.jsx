...
export default function AddToCollectionBtn({ anilistId, mediaType }) {
  ...
  useEffect(() => {
    if (!open || collections.length === 0 || !anilistId) { setMembership({}); return }
    const load = async () => {
      const map = {}
    ...
    try {
      if (membership[collId]) {
        ...
        await supabase.from('collection_items').insert({
          collection_id: collId, anilist_id: anilistId, media_type: mediaType
        })
        ...
        <div className="absolute left-0 top-full mt-2 w-64 bg-background/95 backdrop-blur-xl border border-white/10 rounded-xl shadow-2xl z-50 p-3">
          ...
          {showNew ? (
            <form onSubmit={handleCreateNew} className="mt-2 flex gap-1">
              <input value={newName} onChange={e => setNewName(e.target.value)} placeholder="Nueva colección..." maxLength={100}
                className="flex-1 px-2.5 py-1.5 rounded-lg bg-background border border-white/10 text-xs focus:outline-none focus:border-primary/50 transition-all" autoFocus />
              <button type="submit" disabled={!newName.trim()}
                className="px-2.5 py-1.5 rounded-lg bg-primary text-white text-[10px] font-medium disabled:opacity-40 shrink-0">Crear</button>
            </form>
          ) : (
