  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        {totalSize > 0 && (
          <span className="text-xs text-text-secondary">{formatSize(totalSize)} total</span>
        )}
      </div>

      {downloads.length === 0 && videoDownloads.length === 0 ? (
        <EmptyState
          message="No hay descargas guardadas."
          action={{ label: 'Explorar manga', to: '/manga' }}
        />
      ) : (
            ...
            <div>
              ...
              <div className="space-y-2">
                {videoDownloads.map((d) => (
                  <VideoDownloadCard key={d.id} item={d} onDelete={() => { removeVideoDownload(d.id); refresh() }} />
                ))}
              </div>
            </div>
          )}
            ...
            <div>
              ...
              <div className="space-y-2">
                {mangaDownloads.map((d) => (
                  <DownloadCard key={d.id} item={d} onDelete={() => { removeDownload(d.id); refresh() }} />
                ))}
              </div>
            </div>
          )}
...
function VideoDownloadCard({ item, onDelete }) {
  ...
  useEffect(() => {
    isVideoCached(item.id).then(v => { setCached(v); setChecking(false) })
  }, [item.id])
...
function DownloadCard({ item, onDelete }) {
  ...
  useEffect(() => {
    async function check() {
      if (!item.pages?.length) { setChecking(false); return }
